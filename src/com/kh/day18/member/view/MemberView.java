...
public class MemberView {
		...
		while(true) {
			...
			switch(choice) {
				...
				case 2 : 
					...
					index = controller.findIndexByName(name);	// 인덱스 값을 찾고
					mOne = controller.findOneByName(name);		// 수정하지 않은 부분은 기존 데이터가 유지되도록 정보를 불러옴
					if(index != -1) {				
						this.modifyMember(mOne);	// 데이터가 있을 때 수정할 정보 입력 받기
					}
	...
	public Member inputMember() {	// 반환형을 리턴타입으로 바꿔주기
		Scanner sc = new Scanner(System.in);
		...
		Member member = new Member(memberId, memberPw, memberName, memberEmail, memberPhone);	// 멤버 객체를 만들어서 담기
//		Member member = new Member();
//		member.setMemberId(memberId);
//		member.setMemberId(memberPw);
//		member.setMemberId(memberName);
//		member.setMemberId(memberEmail);
//		member.setMemberId(memberPhone);
		return member;
	}
...
//		Member member = new Member(memberPw, memberEmail, memberPhone);
	...
	public void printAllMembers(List<Member> mList) {	// mList를 사용하기위해 매개변수로 끌어옴
		System.out.println("===== 5. 회원 전체 정보 출력 =====");
